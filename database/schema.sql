...
-- Next steps: Configure RLS policies based on your authentication setup-- Users table for authentication
...
CREATE POLICY users_isolation_policy ON users
    FOR ALL
    USING (auth.jwt() ->> 'company_id' = company_id::text)
    WITH CHECK (auth.jwt() ->> 'company_id' = company_id::text);
...
CREATE POLICY user_sessions_isolation_policy ON user_sessions
    ...
    USING (
        EXISTS (
            ...
            AND auth.jwt() ->> 'company_id' = users.company_id::text
        )
    );
...
CREATE POLICY email_verifications_isolation_policy ON email_verifications
    FOR ALL
    USING (
        EXISTS (
            SELECT 1 FROM users 
